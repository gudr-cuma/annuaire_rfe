...
export async function onRequest(context) {
  ...
  if (!file) return error('Le fichier AGC est requis.', 400);

  const entries = (await readCodeLabelCsv(file)).filter(e => e.code);
  if (entries.length === 0) {
    return error('Fichier AGC : aucune ligne valide (attendu : code;nom, une par ligne).', 400);
  }
  ...
  const db = env.DB;
  try {
    await db.prepare('DELETE FROM agc_ref').run();
    const stmt = db.prepare('INSERT INTO agc_ref (code, nom, imported_at) VALUES (?, ?, ?)');
    for (const batch of chunk(rows, BATCH_CHUNK_SIZE)) {
      await db.batch(batch.map(row => stmt.bind(...row)));
    }
    await upsertImportMeta(db, 'agc', importedAt, rows.length, file.name);
  } catch (err) {
    return error(
      `Erreur pendant l'import du fichier AGC : ${err.message}. La table peut être partiellement vidée — relancez un import complet.`,
      500
    );
  }
